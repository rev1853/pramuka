...
import { socket, Events, setGameConfig, startGame, submitAnswer, leaveRoom, on, fetchCategories } from '../api.js';
...
let lobbyForm = { category: 'all', questionCount: 10, points: 10, penalty: 10, secondsPerQuestion: 20 };
...
function renderGame() {
  ...
  if (reveal) {
    ...
    revealBox = el('div', { class: `p-4 rounded-xl ${color} mt-4` },
      el('p', { class: 'font-semibold' }, title),
      result?.explanation ? el('p', { class: 'text-sm mt-1 text-slate-600' }, result.explanation) : null
    );
  } else if (tooLate) {
    revealBox = el('div', { class: 'p-4 rounded-xl bg-slate-100 text-slate-600 mt-4' },
  ...
  const view = el(
    ...
    card(statusBar, el('h2', { class: 'text-lg font-bold text-slate-900 mb-4' }, question.question), el('div', { class: 'grid gap-3' }, ...optionButtons), revealBox)
  );
